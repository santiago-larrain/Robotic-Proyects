...
c1 = color_HSV_test['red']
c2 = color_HSV_test['blue']
c3 = color_HSV_test['yellow']
...
def filtro(col, fram):
	return cv2.inRange(fram, col - rango, col + rango)
...
while(True):
	...
	try:
		...
		dist = LARGO_AUTO*np.linalg.norm(vector_C1_C3)/np.linalg.norm(vector_C1_C2)
	...
	def click_event(event, x, y, flags, params):
		global c1, c2, c3, count
		if event == cv2.EVENT_LBUTTONDOWN:
			next_color = frame2[y][x]
			if count == 0:
				c1 = next_color
			if count == 1:
				c2 = next_color
	...
	cv2.setMouseCallback('Observador | Original', click_event)
