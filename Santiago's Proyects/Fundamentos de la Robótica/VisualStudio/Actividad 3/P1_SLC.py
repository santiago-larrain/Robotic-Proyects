...
def main(p1_prev, p2_prev,q1_prev, q2_prev, v1a_prev, v2a_prev, v1b_prev, v2b_prev):
    ...
    v1b = R(w1*deltaT).dot(v1b_prev)
    v2b_F2 = np.array([-np.sin(q2/180 * np.pi)*np.linalg.norm(l1_vector)*(w1/180 * np.pi), 
                                                       np.cos(q2/180 * np.pi)*l1*(w1/180 * np.pi) + l2*((w1 + w2)/180 * np.pi)])
    v2b = R(q1 + q2).dot(v2b_F2)
    return p1, p2, q1, q2, v1a, v2a, v1b, v2b
...
if __name__ == '__main__':
    ...
    p1_aux, p2_aux, q1_aux, q2_aux, v1a_aux, v2a_aux, v1b_aux, v2b_aux = main(l1_vector, l2_vector, q1_0, q2_0, np.array([0, 0]), np.array([0, 0]), 
                                            np.array([0, l1*(w1/180 * np.pi)]), np.array([0,0]))
    while True:
        sleep(deltaT)
        p1_aux, p2_aux, q1_aux, q2_aux, v1a_aux, v2a_aux, v1b_aux, v2b_aux = main(p1_aux, p2_aux, q1_aux, q2_aux, v1a_aux, v2a_aux, v1b_aux, v2b_aux)