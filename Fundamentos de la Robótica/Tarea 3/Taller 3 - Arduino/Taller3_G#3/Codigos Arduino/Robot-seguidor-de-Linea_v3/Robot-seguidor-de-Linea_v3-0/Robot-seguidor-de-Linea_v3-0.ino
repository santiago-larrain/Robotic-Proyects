 ...
 * Consideraciones:
 *    - Se observó un error en la función del controlador, pues se multiplicaba
 *    Kd por el periodo cuando se tenía que dividir; esto generaba errores al
 *    incorporar esta constante en el controlador.
 *    - Se hacen múltiples pruebas de velocidad y se varían los parámetros
 *    Kp, Ki y Kd de tal forma que se recorre el circuito lo más rápido posible,
 *    cuidando hacer bien las curvas.
 *    - La velocidad máxima a utilizar es con motorout = 95, pues para velocidades
 *    superiores, el auto no era capaz de responder correctamente a las curvas, apagándose.
 *    Este bug requiere análisis. Una teoría fue que le exigimos un gran cambio de potencia
 *    a los motores, el cual no es capaz de compensar. Se requieren pruebas en circuitos
 *    con curvas más amplias para validar la hipótesis.
 * ----------  UPDATES  -----------
 * > Se establece como los parámetros óptimos para el circuito de prueba:
 *      power_max = 95, Kp = 1.0, Ki = 0.8, Kd = 0.05.
 * > Se arregló el error en la ecuación del controlador para Kd.
 * --------------------------------
 */
...
const int curva = 1;       // 0: Lineal, 1: Tangente, 2: Arcoseno
